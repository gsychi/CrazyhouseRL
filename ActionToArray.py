...
def moveArray(move, board, notArrayToString=True):
    ...
    if "PRNBQ".find(move[0]) == -1:
        ...
        rowNames = "abcdefgh"
        if notArrayToString:
            pickUpPlane[8 - int(move[1])][int(rowNames.find(move[0]))] = 2
...
def moveEvaluation(move, board, prediction):
    ...
    for i in range(len(evaluate)):
        if evaluate[i] != 0:
            # meaningful number...
            if finalEval == 0:
                finalEval = evaluate[i]
                if finalEval < 0:
                    finalEval -= 0.3
            else:
                if finalEval < 0 and evaluate[i] < 0:
                    finalEval = -(finalEval * evaluate[i])
                    finalEval -= 1
                    pickUp = False
                else:
                    finalEval = finalEval * evaluate[i]
                    pickUp = False
        if pickUp and finalEval < 0:
            finalEval = abs(finalEval**2) * -1
        elif pickUp and finalEval > 0:
            finalEval = finalEval ** 2
    """
...
if testing:
    ...
    moveArrayToString(A_MOVE, testingBoard, pythonBoard, whiteCaptivePieces, blackCaptivePieces, plies)
